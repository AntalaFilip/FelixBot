		...
		const collection = new Collection(
			cards.map(
				card => {
					...
					const str = `${card.period.starttime}-${card.period.endtime} > ${card.lesson.subject.short} with ${card.lesson.teachers.map(t => t.member ? removeStartingDot(t.member.displayName) : t.short).join(' & ')} group ${card.lesson.groups.map(g => g.name).join(' & ')}`;
					return [
						card.id,
						str,
					];
