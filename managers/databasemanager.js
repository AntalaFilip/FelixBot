...
class DatabaseManager {
	...
	updateLesson(lesson) {
		...
		db.query(`UPDATE lessons SET students = '${JSON.stringify(lesson.students)}', allocated = '${JSON.stringify(allocated)}' WHERE id = ${lesson.id}`);
	}
	...
	endLesson(lesson) {
		return new Promise((resolve, reject) => {
			...
			db.query(`UPDATE lessons SET endedat = "${this.client.stringUtils.dateToString(lesson.endedAt)}" WHERE id = ${lesson.id}`, err => {
	 ...
	 * @returns The ID of the Lesson, generated by the database.
	 */
	pushNewLesson(lesson) {
		return new Promise((resolve, reject) => {
			...
			this.client.stringUtils.resolveClass(lesson.classid).then((classname) => {
				...
				const allocated = new Array();
				lesson.allocated.forEach(chan => allocated.push(chan.id));
				db.query(`INSERT INTO lessons (teacher, lesson, classname, \`group\`, period, startedat, students, allocated) VALUES ("${lesson.teacher.member.id}", "${lesson.lessonid}", "${classname}", "${lesson.group}", "${lesson.period}", "${this.client.stringUtils.dateToString(new Date(lesson.startedAt))}", '${studentJson}', '${JSON.stringify(allocated)}')`, (err, res) => {
					if (err) throw err;
